...
if "answers" not in st.session_state:
    st.session_state.answers = {"technical": {}, "behavioral": {}}
...
if st.session_state.stage == "greeting":
    ...
    with st.container():
        ...
        with col2:
            ...
            location = st.text_input("📍 Current Location", placeholder="Bengaluru, India")
    ...
    if not st.session_state.tech_questions:
        ...
        for i, q in enumerate(st.session_state.tech_questions, 1):
            st.markdown(f"**Q{i}.** {q}")
            st.session_state.answers["technical"][q] = st.text_area(
                f"Your Answer to Q{i}:", key=f"tech_answer_{i}"
            )
            st.divider()
    ...
    if not st.session_state.behavioral_questions:
        ...
        st.markdown("### 🧠 Behavioral Questions")
        st.markdown("Please answer the following behavioral questions:")

    if st.session_state.behavioral_questions:
        for i, q in enumerate(st.session_state.behavioral_questions, 1):
            st.markdown(f"**BQ{i}.** {q}")
            st.session_state.answers["behavioral"][q] = st.text_area(
                f"Your Answer to BQ{i}:", key=f"behavioral_answer_{i}"
            )
            st.divider()
    ...
    with col2:
        ...
        if st.session_state.answers:
            with st.expander("📄 View Submitted Answers"):
                for i, (q, a) in enumerate(st.session_state.answers["technical"].items(), 1):
                    st.markdown(f"**Q{i}:** {q}\n\n📜 *Answer:* {a or 'Not answered'}")
                for i, (q, a) in enumerate(st.session_state.answers["behavioral"].items(), 1):
                    st.markdown(f"**BQ{i}:** {q}\n\n📜 *Answer:* {a or 'Not answered'}")
